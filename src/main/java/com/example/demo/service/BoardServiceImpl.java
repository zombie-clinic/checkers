...
import static com.example.demo.domain.Side.BLACK;
...
public class BoardServiceImpl implements BoardService {
    ...
    private Map<Integer, List<PossibleMove>> getPossibleMovesForBlack(State state) {
        Map<Integer, List<Position>> blackPositionsAndNeighbors = state.getBlack()
                .stream()
                .collect(toMap(Function.identity(), this::getIJ))
                .entrySet()
                .stream()
                .collect(toMap(Map.Entry::getKey, entry -> getValidNeighborsForPosition(entry.getValue())
                ));


        List<Integer> freeCells = getValidCells();
        freeCells.removeAll(state.getBlack());
        freeCells.removeAll(state.getWhite());

        List<Position> freeCellsPositions = freeCells
                .stream()
                .map(this::getIJ)
                .map(fc -> new Position(fc[0], fc[1]))
                .toList();
        ...
        for (Map.Entry<Integer, List<Position>> pieceNeighbors : blackPositionsAndNeighbors.entrySet()) {
            ...
            for (Position free : freeCellsPositions) {
                if (neighbors.contains(free)) {
                    possibleMoves.computeIfAbsent(piece,
                                    k -> new ArrayList<>())
                            .add(new PossibleMove(BLACK, piece, getBoard()[free.i][free.j], false, false));
                }
            }
